        ...
        if (this.board) {
            this.board.forEach((row, i) => {
                row.forEach((cell, j) => {
                    ...
                    const pieceValue = piece(model.board, position);
                    if (pieceValue !== undefined && pieceValue !== null) {
                        cell.innerText = pieceValue.toString();
                    } else {
                        cell.innerText = ''; // or some default value
                    }
